...
router.get('/getAllEx/:serverid', async (req, res) => {
  try {
    const data = await Model.find({
      serverid: { $ne: req.params.serverid },
    }).select('serverid status -_id');
    res.json(data);
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
...
router.post('/wa', async (req, res) => {
  const { nomor, wa_id, isBusiness, canReceiveMessage, numberExists } =
    req.body;
  try {
...
router.get('/wa/:number', async (req, res) => {
  const number = req.params.number;
  try {
    const data = await Wa.findOne({ nomor: number });
    if (data) {
      return res.status(400).json({ message: 'This Number Already Exists!' });
    }
    return res.status(200).json({ message: 'Account Not Exists!' });
  } catch (error) {
    res.status(500).json({ message: 'An error occurred' });
  }
});
